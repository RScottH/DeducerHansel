...
The code in the files of the above packages is covered by the GPLv2 licenses for those packages.
 ...
 The current file most notably uses code from TransformDialog.java in the Deducer package. 
...
public class HanselTransformDialog extends JDialog implements ActionListener{
	...
	public HanselTransformDialog(String currentDataName, GMDialog diag) {
		if(lastModel==null)
			lastModel = new TransformDialogModel();
                
		initGUI();
                
                variableSelector.setVisible(false);
                addButton.setVisible(false);
                removeButton.setVisible(false);
                
                
                setModel(lastModel,diag);
                
                variableSelector.setSelectedData(currentDataName);

                Object[] sel = variableSelector.getSelectedVariables().toArray();
                
                
                        Object[] elements = diag.variableSelector.getSelectedVariables().toArray();
			String temp;
	...
	private void setModel(TransformDialogModel mod,GMDialog diag){
                dialog = diag;
		try{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(cmd=="Cancel")
			...
			lastModel = getModel();
			String call = getCall();
                        for(int i =0;i<transformVariableList.getModel().getSize();i++){
                            if(!variableSelector.toString().contains(transformVariableList.getModel().getElementAt(i).toString().split("\u2192")[1]))
				dialog.variableSelector.add(transformVariableList.getModel().getElementAt(i).toString().split("\u2192")[1]);
			}
