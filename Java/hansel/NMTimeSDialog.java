...
The code in the files of the above packages is covered by the GPLv2 licenses for those packages.
 ...
 The code from those packages used in this one most notably come from
  GLMDialog.java and ModelBuilder.java, found in the Deducer package
 and
...
public class NMTimeSDialog  extends JDialog implements ActionListener {
	...
	private static NMTimeSDialog  theDialog;
	...
	public static void runitagain(GMModel mod) {
		if(theDialog == null){
			theDialog = new NMTimeSDialog(mod);
		}
	...
	public NMTimeSDialog(JFrame frame) {
		super(frame);
		widgets = new Vector();
		initGUI();
                resetModel();
	...
	public NMTimeSDialog(GMModel mod) {
		super();
...

			{
                                ...
                                }
				{
					...
					newVariablesLabel.setText("New Variable:");
                                        newVariablesLabel.setFont(font12);
	...
	private void completed(){
		...
		if(!valid())
			return;
                ...
                NMTimeSExplorer exp = new NMTimeSExplorer(model,false);
