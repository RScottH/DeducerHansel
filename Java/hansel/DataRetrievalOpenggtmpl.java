...
public class DataRetrievalOpenggtmpl extends FileSelector{
	...
	JTextField proj ;
        ...
        private static DataRetrievalOpenggtmpl theDialog;
	public DataRetrievalOpenggtmpl(Frame f) {
		...
		if (this.isSwing())
		{
			this.getJFileChooser().addChoosableFileFilter(new FileNameExtensionFilter("ggplot2 template file (*.ggtmpl)", "ggtmpl"));
	...
	public DataRetrievalOpenggtmpl() {
		this(null);
	}
          ...
          public static void runit() {
			...
			theDialog = new DataRetrievalOpenggtmpl();
	...
	public boolean runInR(){
		...
		String command = "";
                if(fileName.toLowerCase().endsWith(".ggtmpl")){
                    
                    PlottingElement el = new PlottingElement();
	            el.setFromFile(this.getSelectedFile());
